        ...
        if (isset($_FILES["file"]) && $_FILES["file"]["tmp_name"] != '') { 
            ...
            if ($uploadOk == 1) {
                if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFile)) {
                    ...
                    $sql = "UPDATE menu SET nama = '$nama', harga = $harga, kategori = '$kategori', gambar = '$filename' WHERE id_menu = $id";
            ...
            $harga = $_POST['harga'];
            $kategori = $_POST['kategori'];
            $sql = "UPDATE menu SET nama = '$nama', harga = $harga, kategori = '$kategori' WHERE id_menu = $id";
            if ($conn->query($sql) === TRUE) {
                echo "Record updated successfully";
                header("location:page.php?mod=menu");
            } else {
